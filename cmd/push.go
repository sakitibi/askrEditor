...
// カレントディレクトリの apikey.asenv から APIキーを取得
func getAPIKey() string {
	data, err := os.ReadFile("apikey.asenv")
	if err != nil {
		return ""
	}
...
func PushWiki(wikiSlug string) {
	...
	err = filepath.Walk(wikiSlug, func(path string, info os.FileInfo, err error) error {
		...
		// apiKey を読み込んで指定
		apiKey := getAPIKey()
		resp, err := callAPI(method, wikiSlug, slug, body, accessToken, apiKey)
		if err != nil {
	...
	for slug := range serverSlugs {
		...
		// apiKey を読み込んで指定
		apiKey := getAPIKey()
		resp, err := callAPI("DELETE", wikiSlug, slug, nil, accessToken, apiKey)
		if err != nil {
